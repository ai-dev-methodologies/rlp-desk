...
| `run` | `<slug> [--max-iter N] [--worker-model M] [--verifier-model M] [--mode tmux\|native\|agent] [--worker-engine claude\|codex] [--verifier-engine claude\|codex] [--codex-model MODEL] [--codex-reasoning low\|medium\|high] [--verify-mode per-us\|batch] [--verify-consensus]` | Run the leader loop |
...
The `run` command accepts `--mode tmux|native|agent` (default: `tmux`). Per [ADR-001](../plans/adr-001-leader-consolidation.md):

- **`--mode tmux`** (default, production): Validates the scaffold, checks prerequisites (`tmux`, `jq`), then launches `run_ralph_desk.zsh` as the Leader. The LLM/CLI session exits after launching the script; the shell script runs independently in a tmux session with the full safety net (heartbeat, copy-mode guard, prompt-stall, model upgrade).
- **`--mode native`** (slash-command **only**): The current Claude Code session acts as the Leader, dispatching Workers and Verifiers via `Agent()`. Reachable only through `/rlp-desk run <slug> --mode native` from a Claude Code session — the direct Node CLI does not implement it.
- **`--mode agent`**: **hard-errors (exit 2)**. The direct-CLI Node leader was retired (ADR-001); use `--mode tmux`.
...
Every codex launch (Worker, Verifier, consensus, and all TUI restart/relaunch paths — both leaders) is prefixed with `OMX_STATE_ROOT` pointed at a campaign-scoped `<runtime dir>/omx-state` **root** — actual state lands one level deeper, at `<runtime dir>/omx-state/.omx/state/…`, disjoint from the project's own `.omx/state/`. This isolates each campaign codex session from the operator's interactive omx state (`--disable plugins` does not cover `hooks.json` native hooks, so env isolation is required) — see failure-modes.md F1.18.
