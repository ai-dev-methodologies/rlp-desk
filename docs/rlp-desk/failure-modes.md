...
### F1.18 — stale interactive omx state blocks/wedges campaign codex sessions (fix/omx-state-isolation)
| Field | Value |
|---|---|
| Symptom | A campaign-launched codex worker or verifier blocks/wedges instead of reaching ready, with no leader-side cause visible — the campaign shares the operator's project-local `.omx/state/` with every codex process it spawns, so an unreleased `input_lock` (or other stale state) left behind by the operator's own interactive Claude Code session can be inherited by a campaign codex session. 3 incidents on 2026-08-09 (2 guard blocks + 1 31-minute wedge) |
| Root cause | `oh-my-codex` hooks read project-local `.omx/state/` by default with no per-invocation isolation. `--disable plugins` (already present at every codex launch) covers plugin-registered behavior but does **not** cover `hooks.json` native hooks, so a campaign codex process and the operator's own interactive codex/Claude Code session read and write the *same* state directory (locks, session records) even though they are logically unrelated processes. **Scope (amended 2026-08-10, US-001):** this entry covers *shared-state* contention only — a stale artifact (e.g. an unreleased `input_lock`) written by a **different** process. It does **not** explain hook capture in general. A campaign can be blocked by the very same native-hook surface while `OMX_STATE_ROOT` isolation is working perfectly, because the `UserPromptSubmit` **keyword-detector** activates from the campaign's *own* prompt inside the campaign's *own* isolated root — that is a distinct failure mode, **F1.19**, and stale operator state is not its mechanism. Do not attribute a blocked-write symptom to F1.18 without first checking F1.19 |
| Detection | `tests/test_omx_state_isolation.sh`: grep-pins that `OMX_STATE_DIR` is defined once (derived from `RUNTIME_DIR`) and `mkdir -p`'d in `main()`, and that all 8 `${CODEX_BIN:-codex}` launch-assembly sites in `run_ralph_desk.zsh` carry an `OMX_STATE_ROOT=${(q)OMX_STATE_DIR}` prefix (zsh-quoted for safe shell reinput), plus the `rlp-desk.md` native-leader `Bash("... codex exec ...")` templates AND the `governance.md` native-leader template + tmux-runner illustration (the distributed runtime file, synced to `~/.claude/ralph-desk/governance.md`). `tests/node/us006-campaign-main-loop.test.mjs` (US-006 fix/omx-state-isolation cases): asserts the Node `--mode tmux` leader's codex dispatch command carries `OMX_STATE_ROOT` pointing under the campaign's runtime dir, that the dir is `mkdir -p`'d before the launch, and the no-`omxStateDir` compat fallback (no prefix) |
| Recovery | Every codex session a campaign launches now runs with `OMX_STATE_ROOT` pointed at a campaign-scoped **root** (`<runtime dir>/omx-state`, `mkdir -p`'d before first use) instead of the shared project `.omx/state/` — isolating it from the operator's interactive omx state. `OMX_STATE_ROOT` is a root, not the state dir itself: actual state lands one level deeper, at `<runtime dir>/omx-state/.omx/state/…`. Applied uniformly across both leaders: zsh (`run_ralph_desk.zsh`, all 8 `${CODEX_BIN:-codex}` launch-assembly sites — TUI restart/dispatch/relaunch sends and heredoc trigger scripts — quoted with `${(q)OMX_STATE_DIR}` so a single-quote-bearing path cannot break the command) and Node (`campaign-main-loop.mjs`'s `buildLaunchCommand`, threaded from `paths.omxStateDir` and already shell-quoted via the existing `shQuote` helper, with a no-op fallback for any future non-campaign caller that omits it), plus the native-leader spec (`rlp-desk.md`) `Bash("codex exec ...")` templates and the equivalent `governance.md` templates (native-leader + tmux-runner illustration). An already-wedged campaign is recovered by killing the stuck pane/process and relaunching — the isolation prevents recurrence, it does not itself unstick an in-flight session. **Operator note**: ensure `OMX_ROOT` is not exported in the shell a campaign launches from — it outranks `OMX_STATE_ROOT` at 4 resolution sites in oh-my-codex, so an exported `OMX_ROOT` silently defeats this isolation |
| Reference | `src/scripts/run_ralph_desk.zsh` (`OMX_STATE_DIR`, all `${CODEX_BIN:-codex}` sites); `src/node/runner/campaign-main-loop.mjs` (`buildPaths` `omxStateDir`, `ensureDirs`, `buildLaunchCommand`); `src/commands/rlp-desk.md` (Worker/Verifier/consensus codex exec templates); `src/governance.md` (native-leader template + tmux-runner illustration); `tests/test_omx_state_isolation.sh`; `tests/node/us006-campaign-main-loop.test.mjs`; **F1.19** (the distinct native-hook capture mode) |

### F1.19 — oh-my-codex native hooks capture a campaign codex session from its OWN prompt (US-001)
| Field | Value |
|---|---|
| Symptom | A campaign-launched codex worker or verifier reaches ready and starts working, then every write tool it attempts is refused with a `PreToolUse` block naming deep-interview ("Deep-interview is active…"), until the session aborts. Observed 2026-08-09 (campaign `sv-oracle-nv`, `--mode native`): 11 consecutive blocks, worker aborted, zero files written. Distinct from F1.18 — the session is not wedged and no lock is held; it runs and is refused |
| Root cause | `~/.codex/hooks.json` registers `oh-my-codex/dist/scripts/codex-native-hook.js` on 7 lifecycle events **globally**, and `--disable plugins` does **not** cover native hooks. The `UserPromptSubmit` keyword-detector scans the prompt text of every codex launch; the campaign's own seeded worker prompt contains `Don't assume.` (from `init_ralph_desk.zsh`), which the detector treats as an implicit keyword — it is explicitly exempted from negative-prefix suppression — and auto-activates the deep-interview skill. `readActiveDeepInterviewStateForPreToolUse` then blocks every write tool for the remainder of that codex session. **`OMX_STATE_ROOT` isolation was working and did not prevent this**: the campaign trips the hook with **its own prompt inside its own isolated root** (the `skill-active-state.json` under the campaign-scoped `.../omx-state/.omx/state/sessions/<id>/` names its own cause — `{"skill":"deep-interview","keyword":"Don't assume","source":"keyword-detector"}`, and that session id matches the worker log). Stale operator state is **not** the mechanism, so neither clearing it nor exporting more root vars fixes anything |
| Detection | `tests/node/codex-launch-hook-isolation.test.mjs` (probe predicate, chokepoint routing of all 8 zsh launch-assembly sites, Node parity, native-leader templates, this atlas entry); `tests/test_codex_hook_fallback.sh` (AC7 stale-probe strip-and-retry). Field signature: `Deep-interview is active` in the codex pane/log plus a `skill-active-state.json` with `"source":"keyword-detector"` under the campaign's own `OMX_STATE_ROOT` |
| Recovery | Every campaign codex launch adds **`--disable hooks`** alongside the existing `--disable plugins`, removing the whole native-hook surface (nothing safety-bearing is lost — the surface is keyword/routing/telemetry hooks; the real cost is session provenance for campaign codex runs). The flag is **probe-gated**, because an unknown feature name is a hard error (`codex --disable bogus` → `Error: Unknown feature flag`) that would break every launch on an older CLI: `run_ralph_desk.zsh` probes once at startup for **name presence in column 1** of `codex features list` (never the state column — `apply_patch_freeform` is listed `removed false` yet is still an accepted `--disable` target) against the `command -v codex` absolute path. Escape hatch `RLP_CODEX_HOOKS=1`. A probe that goes stale mid-campaign (operator runs `codex update`) is caught at runtime by the single chokepoint `_codex_launch_with_hook_fallback` (`lib_ralph_desk.zsh`), which all launch-assembly sites route through: on a launch failure whose text names `Unknown feature flag` it retries **exactly once** with the token stripped and clears `_CODEX_NO_HOOKS_FLAG` (`typeset -g`) for the rest of the run; any other failure propagates unmasked. The native leader probes once via Bash before its first dispatch and applies the same retry rule in prose. Node (`command-builder.mjs`) carries the flag as dead-code parity only — it is CLI-unreachable per ADR-001. `OMX_STATE_ROOT` isolation is retained as defence in depth for the `omx` CLI a worker may invoke directly |
| Reference | `src/scripts/run_ralph_desk.zsh` (`_probe_codex_hooks_flag`, `_CODEX_NO_HOOKS_FLAG`, `_CODEX_LAUNCH_FAIL_TEXT`); `src/scripts/lib_ralph_desk.zsh` (`_codex_launch_with_hook_fallback`); `src/node/cli/command-builder.mjs` (`buildCodexCmd`, parity only); `src/commands/rlp-desk.md` + `src/governance.md` (native-leader templates + probe instruction); `tests/node/codex-launch-hook-isolation.test.mjs`; `tests/test_codex_hook_fallback.sh`; F1.18 (the *shared-state* mode this one is often confused with) |

### F1.20 — leader-recovery commits a foreign editor's work (US-002A + US-002B)
| Field | Value |
|---|---|
| Symptom | A `chore(leader-recovery): commit Worker's uncommitted … (Bug #8 F-8)` commit contains a tracked file the Worker never touched — an operator's in-flight edit, or another tool's. Field case: an out-of-band `install.sh` edit landed in recovery commit `dd0e6d9` |
| Root cause | F-8 attributes "dirty vs HEAD, minus `CAMPAIGN_PREEXISTING_DIRTY`" to the Worker, and that snapshot is taken **once, at leader process start** (`run_ralph_desk.zsh`, just before the main loop). Anything dirtied *after* that instant reads as Worker output. The foreign editor is a human or a **native-mode leader**: two zsh campaigns on one checkout are already impossible (`RUNNER_LOCKFILE_PATH` is exclusive per `ROOT_HASH`), while the native leader registers no lock at all — so the defect is a cross-mode / human asymmetry, not zsh-vs-zsh concurrency |
| Detection | `tests/test_bug8_autocommit_robust.sh` scenarios E/F/F2/H/J/J2 drive the real `_bug8_check_synth_allowed` against tmp repos (excluded / still-recovered / mixed / union-no-op / never-widens / relaunch); K + L are structural guards on the fold site and the capture site. `tests/sv-large-campaign/test-f8-commit-slip-recovery.zsh` is the non-regression side. **US-002B (Option D):** `tests/test_bug8_leader_registry.zsh` drives the same real gate through the 4-case registry contract — positive (live foreign entry → carryover), negative control (no entry → F-8 still auto-commits, the case that proves non-vacuity), stale-PID (pruned, no downgrade), self-exclusion (own `$$` never downgrades) — plus fail-closed read, registry mechanics and the native-leader/doc contracts. Field signature: a `leader-recovery` commit whose `--stat` names a file absent from the iteration's done-claim |
| Recovery | `ITER_PREEXISTING_DIRTY` is re-snapshotted at **every worker dispatch** (immediately after `write_worker_trigger`, the last statement before the Worker can touch the tree) using the same `_git_snapshot` + `_git_dirty_base` pair as Gate 3, and F-8 subtracts the **union** `CAMPAIGN_PREEXISTING_DIRTY ∪ ITER_PREEXISTING_DIRTY`. The union is folded **into the `comm -23`, before the `-z` guard** — filtering `_bug8_add` after the array build instead would hit the zsh `(@f)` hazard (an empty string through `"${(@f)var}"` yields **one empty element**, so `${#_bug8_add}` is 1, the D-20 empty-list BLOCK never fires, and execution falls into `git diff --quiet HEAD -- ''`). Union-only, so the baseline can only **narrow** what F-8 stages, never widen it; on a relaunch it restarts empty because the campaign snapshot is re-captured and already covers everything dirty at that moment (D-25). A failed per-iteration snapshot **retains the prior baseline** rather than clearing it — clearing is the fail-open direction, and Gate 3's own fail-closed dirty check BLOCKs the iteration if git is still broken at recovery time. **US-002B (Option D) closes the cross-mode case outright:** every leader registers a per-PID advisory entry at `<root>/${RLP_DESK_RUNTIME_DIR:-.rlp-desk}/logs/.rlp-desk-leaders-<root_hash>.d/<pid>.json` (`schema_version 1.0`, written via `atomic_write` — **not** `set -C`, which would refuse a stale own-PID file — and never reusing the exclusive `RUNNER_LOCKFILE_PATH`), and F-8 **downgrades from auto-commit to `_bug8_record_carryover`** when the registry holds ≥1 LIVE entry whose pid is not its own. Liveness is `kill -0` at read time, PID-based only; the READER unlinks dead entries in the same pass, which is load-bearing rather than belt-and-braces because the native leader is an LLM and is the least reliable party at running its own cleanup. The read sits immediately before `_bug8_autocommit` (after the D-20 already-committed no-op) so the race window is microseconds, and losing the race merely reverts to the pre-Option-D behaviour for one iteration. An unreadable registry is **fail-closed to carryover**, mirroring Gate 3's GIT-FC idiom. Contract: governance §3c; native-leader steps: `rlp-desk.md` Preparation 7a (register, using `$PPID` — each Bash tool call gets a NEW shell, so `$$` would be dead on return and pruned immediately) and loop end (deregister) |
| Residual (NOT fixed) | Option D closes the **cross-mode** case (native↔tmux, including the same-worker-window race Option A cannot reach). What remains: **1. Human editors.** No mechanism detects a person editing the checkout mid-campaign — a human registers nothing. **2. Same-window edit by an UNREGISTERED writer.** Any tool that is neither leader (a second editor, an IDE formatter-on-save, an unrelated script) is still attributed to the Worker inside one worker window. **3. `RLP_DESK_RUNTIME_DIR` path disagreement (AC7h).** `DESK` honours the operator-overridable `RLP_DESK_RUNTIME_DIR`; two leaders that resolve it differently write to **different registries and never see each other**, so Option D becomes a **silent no-op**. That direction is fail-**open** (status quo auto-commit), so it degrades rather than breaks — but if the operator sets that variable they must set it for **both** leaders, or neither. The same silent-no-op class existed via ROOT-derivation divergence (zsh hashed the raw $ROOT string, which preserves symlinks under zsh $PWD, while the native leader hashes the physical git toplevel) — CLOSED 2026-08-10 by canonicalizing ROOT_HASH over `pwd -P`; the residual is now only an operator exporting a custom ROOT that differs from the git toplevel. **4. Native deregistration skipped.** A native leader that never runs its loop-end deregistration leaves a live-PID entry for the rest of its Claude Code session, over-downgrading a later tmux leader to carryover; graceful, never a BLOCK, and the reader's prune reclaims it once the session process exits. **5. Torn/unparseable registry entry.** The reader prunes only entries whose pid PARSES and is dead; an unparseable `.json` is fail-closed to carryover and is NEVER self-healed — it downgrades that root forever until removed by hand (`rm <root>/.rlp-desk/logs/.rlp-desk-leaders-*.d/<file>.json`). Near-unreachable from the two writers (tmp+`mv`, and the reader globs `*.json` so a crashed `.tmp` is never read); graceful, never a BLOCK. **Recommendation for full isolation: run campaigns with `--worktree`**, which covers the human-editor residual too — with the limitation that **`--worktree` is tmux-only** (`run.mjs` forwards `RLP_CAMPAIGN_WORKTREE=1`; only `run_ralph_desk.zsh` consumes it, so `--mode native` gets nothing), and that it relocates `.rlp-desk/`, `.omc/` and installed dependencies, so the operator must re-run their package install in the worktree. It is a documented recommendation, not a default — flipping the default would be a UX break, not a bug fix |
| Reference | US-002A + US-002B, `.omc/plans/manifest-followup-wave.md` §US-002 (Option D spec + AC7a-AC7h); `src/scripts/run_ralph_desk.zsh` (`ITER_PREEXISTING_DIRTY` declaration + dispatch capture + the Gate 3 `comm -23`; `register_leader` at runner-lock acquisition, `unregister_leader` in `cleanup()`, the `_leader_registry_foreign_live` downgrade branch); `src/scripts/lib_ralph_desk.zsh` (`_leader_registry_dir` / `register_leader` / `unregister_leader` / `_leader_registry_foreign_live`); `src/governance.md` §3c; `src/commands/rlp-desk.md` Preparation 7a + loop end; `tests/test_bug8_autocommit_robust.sh`, `tests/test_bug8_leader_registry.zsh`; F1.5 (the Bug #8 gate this scopes) |
