...
| Detection | `tests/test_omx_state_isolation.sh`: grep-pins that `OMX_STATE_DIR` is defined once (derived from `RUNTIME_DIR`) and `mkdir -p`'d in `main()`, and that all 8 `${CODEX_BIN:-codex}` launch-assembly sites in `run_ralph_desk.zsh` carry an `OMX_STATE_ROOT='$OMX_STATE_DIR'` prefix, plus the `rlp-desk.md` native-leader `Bash("... codex exec ...")` templates. `tests/node/us006-campaign-main-loop.test.mjs` (US-006 fix/omx-state-isolation cases): asserts the Node `--mode tmux` leader's codex dispatch command carries `OMX_STATE_ROOT` pointing under the campaign's runtime dir, that the dir is `mkdir -p`'d before the launch, and the no-`omxStateDir` compat fallback (no prefix) |
| Recovery | Every codex session a campaign launches now runs with `OMX_STATE_ROOT` pointed at a campaign-scoped scratch dir (`<runtime dir>/omx-state`, `mkdir -p`'d before first use) instead of the shared project `.omx/state/` — isolating it from the operator's interactive omx state. Applied uniformly across both leaders: zsh (`run_ralph_desk.zsh`, all 8 `${CODEX_BIN:-codex}` launch-assembly sites — TUI restart/dispatch/relaunch sends and heredoc trigger scripts) and Node (`campaign-main-loop.mjs`'s `buildLaunchCommand`, threaded from `paths.omxStateDir`, with a no-op fallback for any future non-campaign caller that omits it), plus the native-leader spec (`rlp-desk.md`) `Bash("codex exec ...")` templates. An already-wedged campaign is recovered by killing the stuck pane/process and relaunching — the isolation prevents recurrence, it does not itself unstick an in-flight session |
| Reference | `src/scripts/run_ralph_desk.zsh` (`OMX_STATE_DIR`, all `${CODEX_BIN:-codex}` sites); `src/node/runner/campaign-main-loop.mjs` (`buildPaths` `omxStateDir`, `ensureDirs`, `buildLaunchCommand`); `src/commands/rlp-desk.md` (Worker/Verifier codex exec templates); `tests/test_omx_state_isolation.sh`; `tests/node/us006-campaign-main-loop.test.mjs` |
