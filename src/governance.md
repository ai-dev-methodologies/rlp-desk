...
2. **Verification-type alternative gate (auto-assigned).** Classifying a US as 검증형 auto-selects the existing RED-불가 exemption plumbing rather than leaving it to per-campaign doctrine: the Worker uses `verify_existing` (§Iterative Autonomy — Existing implementation rule) — run all covering tests, record exit codes, confirm every AC is covered by PASSING tests — and the Worker Process Audit judges on those fresh results, never demanding a `verify_red`/write-test-first sequence. This is the same allowance the confirmation-mode contract (§1f½ / v0.22.3) grants for already-verified code, generalized to a plan-time classification so a verification-type US never trips the TDD-RED mandate as a false FAIL.
   **A verification-type US expects NO commit.** Its deliverable is confirmation that existing behavior holds, so an iteration that changes no files must not produce one — and MUST NOT claim a `commit` step in done-claim.json. An **empty commit** (`git commit --allow-empty`, or any commit whose tree equals its parent's) manufactured to satisfy a commit expectation is a fabricated evidence artifact and an IL-1 breach: it records no work. The Leader rejects it deterministically (§1f½, reason `empty_commit_on_confirmation_claim`) and the fix contract instructs the Worker to DROP the commit — never to create another one.
...
- **Empty-tree commit (anti-fabrication, inverse of the no-op condition)**: when the done-claim carries **no `write_test` step** (the confirmation/replay/verification-type shape — see IL-2¾ §2) and the claimed commit's tree equals its parent's, the claim is a mismatch (`empty_commit_on_confirmation_claim`). An empty commit records no work, so it is evidence of nothing regardless of claim type; the no-`write_test` scope merely bounds the rejection's blast radius, it is not an assertion that the iteration really was verification work (a refactor or doc iteration is build work and is still rejected on an empty commit — intended). Claims carrying a `write_test` step are untouched by this check. **Boundary**: when the claimed commit has no parent to diff against — a root commit, or a shallow clone whose parent is grafted away — the fact is `unknown` and the oracle **accepts**; that is an expected condition, never a git failure. Genuine git errors (unresolvable object, corrupt repo, git binary failure) keep the infra path below.
...
- **Leader is a pane-creation anchor, not a log viewer**, but it is kept at a readable minimum width (`RLP_LEADER_MIN_WIDTH`, default 30) at startup and each iteration — never collapsed. Before any `-h` split it is widened to `RLP_LEADER_SPLIT_WIDTH` (default 110), a target/floor two-knob model: still-too-narrow after the resize attempt degrades and continues at whatever width was actually achieved, as long as it is still >= `RLP_LEADER_DEGRADE_FLOOR` (default 60) — only a resize that lands below the floor is an explicit error (startup → hard exit; in-loop → BLOCKED `infra_failure`). The floor is never stricter than the target: a low-target call (e.g. the `RLP_LEADER_MIN_WIDTH` site) is unaffected by a higher floor.
