...
- Each step includes: what action, which AC, command executed, exit code, summary; verification steps (verify_red/verify_green/verify_e2e/verify_existing/verify) also include `ts` (ISO-8601 UTC run time) — confirmation-mode freshness is judged against it
