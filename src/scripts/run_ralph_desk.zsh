...
handle_worker_exit_codex() {
  ...
  # rewrite the file before archive_iter_artifacts archives it.
  # IMP-10 (closes H2): markLockStart BEFORE chmod (H3 ordering contract) —
  # done-claim is now unlocked at its archival site instead of never.
  _lifecycle_mark_lock_start "${DONE_CLAIM_FILE:t}" "$iter"
...
cleanup() {
  ...
  # IMP-02: the lazily-created 4th consensus pane (codex cross-verifier TUI) is a
  # lifecycle citizen too — without this teardown the codex TUI keeps running (and
  # burning tokens) until the operator kills the tmux session. Mirror the
  # VERIFIER_PANE idiom exactly; guard on presence since the pane is absent for
  # most campaigns.
  if [[ -n "${CONSENSUS_PANE:-}" ]]; then
    tmux send-keys -t "$CONSENSUS_PANE" C-c 2>/dev/null
    tmux send-keys -t "$CONSENSUS_PANE" "/exit" C-m 2>/dev/null
  fi
  sleep 2
  ...
  if [[ -n "${CONSENSUS_PANE:-}" ]]; then
    tmux kill-pane -t "$CONSENSUS_PANE" 2>/dev/null
  fi
      ...
      if (( _a4_blocked )); then
        ...
        if [[ -n "$dc_us_id" && "$dc_us_id" != "null" ]]; then
          ...
          if _bug8_check_synth_allowed "$ITERATION" "$dc_us_id" "inline_polling_a4_clean"; then
            ...
            # IMP-10 (closes H2): markLockStart BEFORE chmod (H3 ordering
            # contract) — done-claim is now unlocked at its archival site.
            _lifecycle_mark_lock_start "${DONE_CLAIM_FILE:t}" "$ITERATION"
            _lock_sentinel "$DONE_CLAIM_FILE" || true
...
run_single_verifier() {
  ...
  if [[ "$engine" = "codex" ]]; then
    ...
    verifier_launch="OMX_STATE_ROOT=${(q)OMX_STATE_DIR} ${CODEX_BIN:-codex} -m $_cx_model -c model_reasoning_effort=\"$_cx_reason\" -c mcp_servers='{}'${_CODEX_NO_UPDATE_FLAG} --disable plugins --dangerously-bypass-approvals-and-sandbox"
...
_final_verify_one_us() {
  ...
  if [[ "$FINAL_VERIFIER_ENGINE" = "codex" ]]; then
    ...
    verifier_launch="OMX_STATE_ROOT=${(q)OMX_STATE_DIR} ${CODEX_BIN:-codex} -m $FINAL_VERIFIER_CODEX_MODEL -c model_reasoning_effort=\"$FINAL_VERIFIER_CODEX_REASONING\" -c mcp_servers='{}'${_CODEX_NO_UPDATE_FLAG} --disable plugins --dangerously-bypass-approvals-and-sandbox"
...
run_consensus_verification_parallel() {
  ...
  codex_launch="OMX_STATE_ROOT=${(q)OMX_STATE_DIR} ${CODEX_BIN:-codex} -m $_cx_model -c model_reasoning_effort=\"$_cx_reason\" -c mcp_servers='{}'${_CODEX_NO_UPDATE_FLAG} --disable plugins --dangerously-bypass-approvals-and-sandbox"
  ...
  # IMP-02: consecutive-dead-tick counters for the scoped mid-round pane liveness
  # check (2 ticks shields a transient tmux display-message hiccup).
  local _cl_dead=0 _cx_dead=0
  while true; do
    ...
    if (( ! _claude_ok )); then
      # IMP-02: scoped mid-round liveness — a claude verifier pane that dies while
      # its verdict is unresolved is classified immediately (2 consecutive dead
      # ticks) via the infra-failure return, instead of burning the submit-anchored
      # timeout / re-dispatch windows with a misattributed reason.
      if ! _verify_pane_alive "$VERIFIER_PANE"; then
        if (( ++_cl_dead >= 2 )); then
          VERIFIER_ABORT_REASON="Parallel consensus: claude verifier pane died mid-round (verdict unresolved)"
          log_error "$VERIFIER_ABORT_REASON"
          return 1
        fi
      else
    ...
    if (( ! _codex_ok )); then
      # IMP-02: scoped mid-round liveness for the consensus (codex) 4th pane — same
      # immediate infra-failure classification as the claude side above.
      if ! _verify_pane_alive "$CONSENSUS_PANE"; then
        if (( ++_cx_dead >= 2 )); then
          VERIFIER_ABORT_REASON="Parallel consensus: consensus pane died mid-round (codex verdict unresolved)"
          log_error "$VERIFIER_ABORT_REASON"
          return 1
        fi
      else
  ...
  for (( ITERATION = 1; ITERATION <= MAX_ITER; ITERATION++ )); do
    ...
    while (( ! worker_poll_done )); do
      ...
      if poll_for_signal "$SIGNAL_FILE" "$WORKER_HEARTBEAT" "$WORKER_PANE" "$worker_launch" "Worker"; then
        ...
        # archive (archive_iter_artifacts) read a snapshot the worker can no
        # longer revise. Symmetric with iter-signal/verdict lock contract.
        # IMP-10 (closes H2): markLockStart BEFORE chmod (H3 ordering
        # contract) — done-claim is now unlocked at its archival site instead
        # of never; the return code remains genuinely unused, explicit.
        _lifecycle_mark_lock_start "${DONE_CLAIM_FILE:t}" "$ITERATION"
        _lock_sentinel "$DONE_CLAIM_FILE" || true
