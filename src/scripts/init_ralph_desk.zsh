...
print_run_presets() {
  ...
  if [[ $codex_available -eq 1 ]]; then
    ...
    echo "/rlp-desk run $slug --mode tmux --worker-model gpt-5.6-terra:medium --consensus final-only --debug"
    echo ""
  ...
  echo "#   --worker-model MODEL                   haiku|sonnet|opus or gpt-5.6-sol:high|terra:medium|spark:high (default: haiku)"
  ...
  echo "#   --consensus-model MODEL                per-US cross-verifier (default: gpt-5.6-terra:medium)"
  echo "#   --final-consensus-model MODEL          final cross-verifier (default: gpt-5.6-sol:high)"
