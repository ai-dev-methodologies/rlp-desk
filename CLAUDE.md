...
docs/rlp-desk/verification-history.md        → ~/.claude/ralph-desk/docs/rlp-desk/verification-history.md
...
- **Lifecycle observability (v0.15.4+, ALWAYS ON since v0.22.4 — the `RLP_LIFECYCLE_METRICS` opt-out flag was removed)**: `src/node/util/lifecycle-metrics.mjs` is the B4 emitter. All 5 metrics are wired on both leaders unconditionally. Emit sites in `src/node/runner/campaign-main-loop.mjs` (Node leader) and `src/scripts/lib_ralph_desk.zsh` `log_lifecycle_metric` / `_lifecycle_emit_write_to_read` / `_lifecycle_mark_lock_start` / `_lifecycle_mark_unlock` (zsh leader).
