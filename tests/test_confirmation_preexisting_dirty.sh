...
# the real code.
# (IMP-09 updated the capture to the fail-closed _git_snapshot helper, and
# A-4 further routed it through _git_dirty_names (the -z + unquote wrapper
# around _git_snapshot) — match the assignment itself, not one specific git
# invocation form.)
cap_line=$(grep -n 'CAMPAIGN_PREEXISTING_DIRTY=\$(_git_dirty_names\|CAMPAIGN_PREEXISTING_DIRTY=\$(_git_snapshot\|^  CAMPAIGN_PREEXISTING_DIRTY=\$(git' "$RUN" | head -1 | cut -d: -f1)
...
[[ "$(grep -c 'CAMPAIGN_PREEXISTING_DIRTY=\$(_git_dirty_names\|CAMPAIGN_PREEXISTING_DIRTY=\$(_git_snapshot\|^  CAMPAIGN_PREEXISTING_DIRTY=\$(git' "$RUN")" == "1" ]] \
