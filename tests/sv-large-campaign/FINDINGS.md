...
## F-17 — codex verifier over-strict on the verifier prompt's meta-gates (doesn't converge)  · OPEN (codex verifier)
| | |
|---|---|
...
| Fix direction | Soften/clarify the verifier prompt's meta-gates for cross-engine consistency (don't block PASS on layer-section formatting or an auto-generated signal when the AC evidence is green), or scope the strict gates to claude. Needs careful prompt design — deferred. |
| Status | OPEN — codex-verifier follow-up; NOT blocking (claude verifier + codex worker proven). |
