...
| Status | report-only, production fix DEFERRED: (a) PRE-EXISTING, unchanged by NEW-4 (on relaunch HEAD exists so NEW-4's empty-tree path isn't taken); (b) BENIGN — the fresh-context Worker re-does that US and commits it; excluding a prior-segment's abandoned partial file from auto-commit is defensible. A real fix persists the campaign-start snapshot across relaunches (status.json) — a feature change that risks re-introducing the F-19 operator-edit sweep; deferred. |
| Resolution (ralplan consensus, test-only) | **The production fix stays DEFERRED; a TEST-ONLY regression guard was ADDED** (ralplan Planner→Architect→Critic=codex, unanimous APPROVE). The F-19 invariant ("operator pre-existing edits are NEVER swept into a Worker-recovery commit") was pinned only WITHIN a single process (test-f8 B/C); the cross-relaunch boundary — exactly where D-25 lives — was unguarded. Added 4 assertions to `test-dbacklog.zsh` (now 116/116): a structural guard ANCHORED to the extracted F-8 scoping line (not a whole-runner grep — the comm -23 subtraction of CAMPAIGN_PREEXISTING_DIRTY), a behavioral replay that SOURCES the real extracted F-8 line (the production code path, not a reimplementation) + the real `_git_dirty_base` in a temp repo simulating a relaunch (snapshot re-captured with operator + prior-segment files), and a negative control (with an empty snapshot the same real F-8 line scopes the operator file IN → guard is load-bearing). codex P2 (whole-runner grep + reimplementation) fixed by anchoring both checks to the extracted production line. **INVARIANT-framed, NOT quirk-framed** (codex Critic, decisive): asserts the operator file stays OUT of the recovery commit + stays dirty; deliberately asserts NOTHING about the prior-segment file, so a future persist-snapshot root fix (which may legitimately commit it) will still pass. Zero production diff; `tests/` is npm-excluded → zero user-facing/tarball impact. |
...
### R3-2 (FIXED, ralplan backlog — defensive Node parity port) — Node leader (campaign-main-loop.mjs) verdict-read parity  · MEDIUM (latent; production-dead path)
| | |
|---|---|
| Symptom | The Node leader's two verdict consumers — the per-US `runFinalSequentialVerify` loop and the main-loop verifier dispatch — both `pollForSignal(paths.verdictFile)`. `pollForSignal` returns the FIRST parseable JSON it reads with NO mtime/freshness gate (signal-poller.mjs:188-199 + last-chance re-read :253-258), and the verdict file is left on disk after consumption (loop-top only `unlockSentinelFile()`s — chmod 0o644 — without unlinking; the reaper kills the producer pane, not the file). So the next dispatch's poll could immediately return the PREVIOUS US's verdict (cross-contamination) or a finalize iteration could accept a leftover verdict — the Node analog of R3-1. |
| Status | FIXED (was previously documented NON-ACTIONABLE because campaign-main-loop is production-dead for `--mode tmux` → zsh leader; `--mode agent` hard-errors; `--mode native` slash-only). The ralplan `--critic codex` backlog elected to port it anyway as DEFENSIVE PARITY + a pinned-contract test, so a future Node-leader revival can't regress here. New module helper `clearStaleVerdict(...verdictFiles)` = per path `unlockSentinelFile` then `fs.rm(force:true)` (unlock-then-unlink per the documented fs.mjs cleanup pattern), called before BOTH `dispatchVerifier` sites (mirrors zsh R3-1 `rm -f "$VERDICT_FILE"`). |
| codex round | codex (gpt-5.5, high) found 2 real gaps in the first cut, both fixed: **HIGH** — the main-loop poll passes `legacySignalFile: paths.legacyVerdictFile`, which the poller reads with the same no-freshness last-chance read (signal-poller.mjs:267-274); clearing only the canonical path left a stale-legacy hole → helper made variadic, both sites now clear canonical **and** legacy. **MEDIUM** — the blanket `.catch(()=>{})` swallowed non-ENOENT unlink failures (stale verdict survives → consumed); removed, so only a missing file is a no-op (via `force:true`) and any real failure propagates (fail loud > mis-verify) — a deliberate stricter divergence from zsh's silent `2>/dev/null`. |
| Verification | New `tests/node/test-r3-2-clear-stale-verdict.test.mjs` (6 tests) pins both halves with PRODUCTION code paths: real `pollForSignal` returns the stale verdict immediately (proves no freshness gate), and after `clearStaleVerdict` it TIMES OUT waiting for a fresh write (proves the clear works); plus remove-locked, no-op-on-missing+falsy-skip, clears-both-canonical+legacy, and non-ENOENT-propagation contracts. `npm run test:node` 393/393 (387 baseline + 6). codex re-review: 0 issues. Purely additive, behavior-neutral on the production zsh path. |

### D-26 (FIXED — production zsh analog of R3-2 HIGH, found via cross-pollination) — legacy verdict never cleared  · MEDIUM
| | |
|---|---|
| Symptom | The zsh leader (PRODUCTION `--mode tmux`) keeps `VERDICT_FILE` (canonical) + `LEGACY_VERDICT_FILE` (pre-v0.13.0 path a codex verifier sometimes writes to). `check_no_progress` → `_verifier_pane_has_verdict` → `_migrate_legacy_verdict` does `mv LEGACY→canonical` during a verifier wait when canonical is absent. All 5 pre-launch/iteration-cleanup clears did `rm -f "$VERDICT_FILE"` but **none removed `LEGACY_VERDICT_FILE`** (grep: zero `rm` of legacy anywhere — only the `mv` in migrate). So a stale legacy verdict left from a prior iteration (codex double-wrote, or a SKIP_NEXT_WORKER/finalize iter skipped cleanup — the same R3-1 trigger) is promoted into THIS iteration's canonical and read by `poll_for_signal` (canonical-only) → wrong pass/fail. R3-1 fixed only the canonical half; this is the legacy half, in the path that actually runs. |
| Fix | Added `"$LEGACY_VERDICT_FILE"` to all 5 clear sites (run_single_verifier pre-launch; run_sequential_final_verify + retry; iteration-start cleanup; inline single-verifier R3-1 site). Mirrors R3-2's canonical+legacy clear, in zsh. |
| codex round | codex (gpt-5.5, high) found 3 real follow-ups, all fixed: **HIGH** — `_final_verify_one_us` cleared the verdict AFTER `launch_verifier_*` (a pre-existing R3-1-era ordering), so a fast verifier's FRESH verdict could be deleted; both clears moved to BEFORE their launch/relaunch. **MEDIUM** — the codex-engine poll loop in `run_single_verifier` watched canonical only and never migrated a FRESH legacy verdict (relied on the no-progress watcher's side-effect); added `[[ -f "$VERDICT_FILE" ]] || _migrate_legacy_verdict` at the loop top (safe because the pre-launch clear removes any stale legacy first). **LOW** — added a test asserting the clear precedes the launch in `_final_verify_one_us`. |
| Verification | test-dbacklog +6 (now **112/112**): structural regression guard (0 `rm $VERDICT_FILE` sites omit legacy; ≥5 paired) + clear-before-launch ordering assertion + codex-loop migrate assertion + a BEHAVIORAL test that extracts & sources the REAL `_migrate_legacy_verdict` (proves stale-legacy promotion = bug, and that the clear closes it). sv-gate:fast 71/71, lock 9/9, zsh -n OK, node 393/393 (unaffected). codex re-review (gpt-5.5 high, fresh ctx): 0 issues. **real-LLM dogfood** (f6mini, isolated `tmux -L` server, SRC runner via `RLP_DESK_ZSH_RUNNER`): COMPLETE in 3 iters — per-US verdict clear/harvest ×3 (verifier caught a `'medium'` vs PRD `'med'` mismatch → fix loop) + D-16 final-verify (clear-before-launch ordering); `poll_failed`/stale-verdict/mismatch = 0, no regression. (legacy-migrate path is codex-verifier-only → 0 events here, covered by the behavioral test.) Found by checking whether codex's R3-2 HIGH (Node legacy-path gap) had a production-zsh twin — it did. |

### D-27 (FIXED — found via D-26 dogfood) — node `run.mjs init` shipped placeholder prompts  · MEDIUM (Node-CLI-only; production /rlp-desk init unaffected)
| | |
|---|---|
| Symptom | The D-26 dogfood failed spuriously when init'd with `node src/node/run.mjs init` (vs the ISOLATION RECIPE's "init via SRC zsh"). `campaign-initializer.mjs` (`buildWorkerPrompt`/`buildVerifierPrompt`) bakes 1-line PLACEHOLDER prompts ("Independent verifier for Ralph Desk: <slug>") with ZERO governance, so a CLI init→run verifier wrote its verdict to a self-chosen path (`iter-NNN.verifier-result.json`) instead of `$SLUG-verify-verdict.json` → leader polled `verify-verdict.json` forever → `poll_failed`. ASYMMETRIC with RUN (v0.14.0 inverted run to delegate to the zsh runner, but init still used node initCampaign). Production npm users are UNAFFECTED — `/rlp-desk init` (src/commands/rlp-desk.md) runs `init_ralph_desk.zsh` directly; only someone driving the Node CLI init→run is bitten, silently. |
| Fix | `runInit` now delegates to `init_ralph_desk.zsh` (symmetric with run's `--mode tmux` → `run_ralph_desk.zsh`), via `defaultZshInitPath`/`defaultSpawnZshInit` + injectable `spawnZshInit`/`zshInitPath` deps. `initCampaign` stays for the in-process Node leader + test fixtures. Objective passed as a SINGLE joined arg (option-looking words preserved); advanced init flags not exposed (matches pre-D-27 node init). |
| codex round | codex (gpt-5.5, high) — **4 review rounds, every finding fixed to 0**: r1 P2 option-looking-objective split + P2 ROOT not pinned → objective single-arg + `ROOT: deps.cwd`. r2 P2 slug normalization lost (`../../outside`) → `normalizeSlug` exported+reused; P3 flag-shaped objective → reject with clear error (no `--` sentinel in zsh init). r3 P2 delegation exposes the init heredoc corruption (→ D-28); P3 signal exit code collapsed to 128 → `os.constants.signals[signal]` 128+signo. r4 P2 `RLP_DESK_RUNTIME_DIR` validation lost → `resolveDeskRoot()` guard before spawn. Final review: **0 actionable regressions**. |
| Verification | `tests/node/test-d27-init-delegate.test.mjs` 14/14 (delegation mapping, objective join, option-looking-objective preserved, ROOT pin, slug normalize, runtime-dir reject, flag-objective reject, missing-runner error, signal/exit passthrough, --help). node full 405–406 pass (only the pre-existing US-003 pollForSignal live-tmux flake — confirmed identical on stashed baseline, NOT a regression). **real-LLM dogfood**: node `run.mjs init` (delegation, `RLP_DESK_ZSH_INIT_RUNNER`) → run → COMPLETE (the exact path that failed in D-26). codex re-review: 0 issues. |

### D-28 (FIXED — surfaced by D-27 delegation, codex r3) — worker prompt heredoc executed markdown backticks  · MEDIUM (also affects production /rlp-desk init)
| | |
|---|---|
| Symptom | `init_ralph_desk.zsh`'s worker prompt heredoc is unquoted (`<<EOF`). Its markdown inline-code spans were written as BARE backticks (`` `lane_violation_warning` ``), which the unquoted heredoc executed as command substitutions while scaffolding — emitting "command not found" noise AND silently STRIPPING the backticked governance terms (`lane_violation_warning`, `infra_failure`, `meta.blocked_hygiene_violated=true`) from the generated worker prompt. Affects production `/rlp-desk init` too (it runs the same script), degrading worker-prompt fidelity (campaigns still COMPLETE — the stripped terms aren't load-bearing). D-27's delegation surfaced it on the Node-CLI path (codex r3). |
| Fix | Escape ONLY the worker heredoc's BARE backticks (`` ` `` → `` \` ``), leaving variable expansion ($DESK/$SLUG/$OBJECTIVE) and the already-escaped fences intact. Minimal (4 lines). Verifier/testspec heredocs were left UNTOUCHED — analysis showed bare-backtick=0 there (their backticks were already `\``-escaped), so they were never corrupted; quoting them (an earlier over-broad cut) would have broken their escaped fences + needed sed-metachar escaping for $DESK/objective paths — both avoided. |
| codex round | codex (gpt-5.5, high): an earlier quoted-heredoc+placeholder cut drew 3 P2s (literal `\`` fences in verifier/testspec, $DESK sed-metachar escaping, RLP_DESK_RUNTIME_DIR). The minimal bare-backtick-escape cut (worker-only, no sed) sidesteps all of them. Final review: **0 actionable regressions**. |
| Verification | `tests/test_d28_heredoc_quoting.sh` 8/8: no command-not-found, backticked terms preserved, vars still expand, objective special chars (`\| & \\ %`) intact (no sed → no escaping), verifier/testspec scaffolds correct, testspec fences intact. `test_us006_init_presets` 12/12, `sv-gate:fast` 71/71 unaffected; `test_a14a15_init_improve` FAIL 2 is PRE-EXISTING (backup-compared: pre-D-28 == with-D-28). zsh -n OK. **SV gate (init_ralph_desk.zsh changed → MANDATORY, CLAUDE.md)**: LOW(f6mini)/MEDIUM(kvstore)/CRITICAL(safepath, opus) all **COMPLETE** via delegated init + D-28 src in isolated `tmux -L` servers (caller untouched). |
