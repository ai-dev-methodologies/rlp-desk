...
const scratchRoot = path.join(repoRoot, '.tmp', 'us00-bootstrap-tests');

async function resetScratchDir() {
  await fs.rm(scratchRoot, { recursive: true, force: true });
  await fs.mkdir(scratchRoot, { recursive: true });
}

test.beforeEach(async () => {
  await resetScratchDir();
});
...
test('US-00 AC2 happy: writeFileAtomic creates a new file under the repo root', async () => {
...
test('US-00 AC2 boundary: writeFileAtomic overwrites existing content and leaves no tmp file behind', async () => {
  const { writeFileAtomic } = await import('../../src/node/shared/fs.mjs');
